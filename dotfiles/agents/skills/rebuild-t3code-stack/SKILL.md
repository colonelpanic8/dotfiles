...
Read `/srv/dotfiles/dotfiles/agents/project-guides/t3code-pr-stack.md` for
locations and safety invariants, then read and follow **`stack/BUILDING.md` on
`fork/t3code/stack-tooling`** — the complete, authoritative procedure lives
...
Use `$inventory-t3code-pr-stack` for a read-only carriage check and
...
- Never commit to, base work on, or merge back `t3code/stack`.
- Never push a stack that has not passed the content audit.
