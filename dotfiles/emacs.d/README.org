...
Install and load the packages required by the startup appearance before the
rest of the configuration queues optional packages.
#+BEGIN_SRC emacs-lisp
...
#+begin_src emacs-lisp
(use-package gcmh
  :ensure (gcmh :host gitlab :repo "koral/gcmh" :protocol https)
  :defer 5
  :config (gcmh-mode 1))
#+end_src
** diminish
...
(use-package emacs-everywhere
  ...
  :config
  (progn
    (defun imalison:hyprland-json-command (command)
      (condition-case nil
          (json-parse-string
           (shell-command-to-string command)
           :object-type 'alist
    ...
    (defun imalison:emacs-everywhere-app-info-hyprland ()
      ...
      (require 'seq)
      (let* ((active-window
              (imalison:hyprland-json-command "hyprctl -j activewindow"))
             (window
              (if (alist-get 'address active-window)
                  active-window
                (seq-find
                 (lambda (candidate)
                   (and (equal (alist-get 'focusHistoryID candidate) 0)
                        (alist-get 'mapped candidate)
                        (not (alist-get 'hidden candidate))))
                 (imalison:hyprland-json-command "hyprctl -j clients"))))
...
* Utility
** dired-sort-menu
Add a menu and dialogue for changing Dired's sort order.
#+begin_src emacs-lisp
(use-package dired-sort-menu
  :ensure (dired-sort-menu
           :host github
