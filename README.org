...
The local =just switch= recipe wraps =nixos-rebuild switch --flake ".#"=. When
called from a Paseo-hosted process it detaches into a dedicated tmux server so
the activation can safely restart Paseo. Use it instead of running
=nixos-rebuild= directly.

Use =fleet-switch= to start the same operation locally or over SSH. Multiple
hosts run concurrently in tiled tmux panes; =local= means the current machine,
and =--online= selects this machine plus configured NixOS hosts currently
online in Tailscale.
